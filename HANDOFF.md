...
3. **Point rachelarre.com at Netlify.** Correction (2026-08-31, verified via ICANN WHOIS): DNS is actually managed through **WordPress.com** (nameservers `NS1/2/3.WORDPRESS.COM`), not Flywheel as originally noted — Flywheel and WordPress.com are separate companies. Rachel confirmed she wants to fully replace the WordPress site (not run them side by side) and has WordPress.com login access. Custom domain `rachelarre.com` + alias `www.rachelarre.com` already added on the Netlify side (site `rachelarre`, project ID `36b01a2e-06fc-42fb-a390-754c4f4227b6`) via API. Records needed at WordPress.com (Domains → rachelarre.com → DNS Records): `A @ → 75.2.60.5` and `CNAME www → rachelarre.netlify.app`. Leave MX/TXT/email records untouched. Once live, Netlify auto-provisions HTTPS.
